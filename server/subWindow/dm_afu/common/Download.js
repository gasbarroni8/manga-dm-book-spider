...
const {  selfmkdir, escapeSpecChart, suagent, forEach$list, cookieToStr, execUrlParams, linkResourceisOk, getHeaderCookie } = require('../../../lib')
	...
	getVideoBigAddress (urlAsId, pageIndex) {
		...
		return new Promise(resolve => {
			...
			;(function intor (idx, cookie={}) {
				...
				let urim = `${HOST}/_getplay?aid=${that.id}&playindex=${ma[0]}&epindex=${ma[1]}&r=`+rand
				...
				suagent(urim, {
					...
					if (text == 'err:timeout') {
						...
						let tempCookieMap = getHeaderCookie(res.headers['set-cookie'], true);
						setTimeout(() => {
							intor(idx + 1, getTempCookie({
								...cookie,
					...
					/* let mtype = ismp4(getUrlFileType(videourl)) ? 'neos' : 'ts';
					if (shtype && mtype != shtype) {
						console.log('因为格式不一样，所以开始重新获取。获取到的是这个', videourl)
						intor(0)
						return;
					} */
			...
			/* ;(function intor (count) {
				...
				let urim = `${HOST}/_getplay?aid=${that.id}&playindex=${ma[0]}&epindex=${ma[1]}&r=`+Math.random()
				console.log('外层下载地址，', urim)
				suagent(urim, {
					setMap: that.getTempSetMap(urlAsId)
				}).then(({text}) => {
					if (txt == 'err:timeout') {
