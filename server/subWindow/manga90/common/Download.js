...
const { saveFile, setRandTimeout, selfmkdir, escapeSpecChart, suagent, forEach$list, execParams, getIdByLastItem, addHttp } = require('../../../lib')
...
function execPicList (text, picHost) {
	...
	forEach$list($('script'), ($item) => {
		...
		if (html.indexOf('chapterImages') != -1) {
			...
			imgPath = chapterPath.split('/').filter(it => !!it).join('/')
			return false;
		}
	})
