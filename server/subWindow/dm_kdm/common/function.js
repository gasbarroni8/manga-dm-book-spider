...
const { suagent, escapeSpecChart, getIdByLastItem, execUrlParams } = require('../../../lib')
...
exports.search = (searchval='', page=1) => {
	...
	return suagent('post', url, {
		...
		for (let i = 0, len = $lis.length; i < len; i++) {
			...
			let id = $tmp.attr('href').replace(/\//ig, '')
			list.push({
				id,
		...
		console.log(list)
		let total = parseInt(execUrlParams($mars.eq($mars.length - 1).attr('href') || '').page) || 0
		console.log('11111')
		return {
			list,
			total: total * 10,
