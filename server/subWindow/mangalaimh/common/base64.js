...
//获取当前图片
function getUrlpics(PicUrls, curid) {
  // var PicUrls = picTree
  currentChapterid = curid

  if (PicUrls.indexOf('mh160tuku') == -1) PicUrls = base64_decode(PicUrls)
  ...
  return PicUrlArr.map((it) => decodeURIComponent(getcurpic(it)))
}
...
function getremoteqqurl(qqurl) {
  ...
  var u =
    'http://img11.aoyuanba.com/pictmdown.php?p=' +
    base64_encode(v) +
    '&sf=' +
    qqfilename +
    '&ym=' +
...
function getcurpic(v) {
  ...
  if (v.indexOf('qq.com/store_file_download') != -1) {
    ...
    s =
      'http://imgsty1.aoyuanba.com/pictmdown.php?bu=' +
      bym +
      '&p=' +
      base64_encode(v1)
    ...
    //alert(v);
    s =
      'http://imgsty.aoyuanba.com/pictmdown.php?bu=' +
      bym +
      '&p=' +
      base64_encode(v)
    ...
    s =
      'http://img7.aoyuanba.com/picinc/qTcms.Pic.FangDao.asp?p=' +
      base64_encode(v)
    ...
    s =
      'http://img8.hgysxz.cn/picinc/qTcms.Pic.FangDao.asp?p=' + base64_encode(v)
