...
class Download extends AllDownload {
  ...
  constructor(id) {
    ...
    this.getListUrl = HOST + '/fzdm/' + this.id + '.html'
    // this
  }
  ...
  execList(text) {
    ...
    const list = []
    const $as = $('.deanluntan_view').find('.deanmedia_jq').find('.panel')
    // const $namelis = $('.container .row .nav-tabs').eq(0).find('li')

    forEach$list($as, ($it, idx) => {
      let tempMap = {
        // name: '线路'+(idx+1),
        name: escapeSpecChart($it.find('h4').text()),
        unclearTotal: true,
        arr: [],
      }
      // tempMap.name = '线路'
      ...
      forEach$list($it.find('.playlist').find('li'), ($li) => {
        const $a = $li.find('a')
...
function getDownloadList(urlAsId) {
  ...
  let allListUrl = `${HOST}/ass.php?url=dp&vid=${mit[0]}&vfrom=${
    mit[1]
  }&vpart=${mit[2]}&cb=${cbName}&_=${Date.now()}`
